...
class AdminPanel {
    ...
    async getBibleVerse(category = 'general') {
        const verses = {
            worship: [
                'psalms 95:1', 'psalms 150:6', 'john 4:24', 'psalms 100:2', 
                'revelation 4:11', 'psalms 96:9', 'hebrews 13:15', 'psalms 103:1'
            ],
            fellowship: [
                'hebrews 10:25', 'proverbs 27:17', '1 john 1:7', 'galatians 6:2',
                'ecclesiastes 4:12', 'romans 12:10', '1 thessalonians 5:11', 'philippians 2:2'
            ],
            events: [
                'deuteronomy 31:6', 'joshua 1:9', 'isaiah 40:31', 'philippians 4:13',
                '2 timothy 1:7', 'jeremiah 29:11', 'proverbs 3:5-6', 'psalms 118:24'
            ],
            service: [
                'mark 16:15', 'matthew 28:19', '1 peter 4:10', 'galatians 5:13',
                'matthew 20:28', 'john 13:14', 'romans 12:1', '2 corinthians 9:7'
            ]
        };

        const categoryVerses = verses[category] || verses.fellowship;
        const randomVerse = categoryVerses[Math.floor(Math.random() * categoryVerses.length)];
        ...
        try {
            ...
            console.log('Primary Bible API failed, trying fallback...');
        }

        try {
            // Try secondary API with different format
            const verseFormatted = randomVerse.replace(/\s+/g, '%20');
            const response = await fetch(`https://labs.bible.org/api/?passage=${verseFormatted}&type=json`);
            if (response.ok) {
                const data = await response.json();
                if (data && data[0]) {
                    return {
            ...
            console.log('Secondary Bible API failed, using static content...');
        }
    ...
    getStaticVerse(category) {
        const staticVerses = {
            ...
            service: [
                { reference: 'Mateo 28:19', text: 'Por tanto, id, y haced discípulos a todas las naciones.' },
                { reference: 'Marcos 16:15', text: 'Id por todo el mundo y predicad el evangelio a toda criatura.' },
                { reference: '1 Pedro 4:10', text: 'Cada uno según el don que ha recibido, minístrelo a los otros.' }
            ]
        };

        const verses = staticVerses[category] || staticVerses.fellowship;
